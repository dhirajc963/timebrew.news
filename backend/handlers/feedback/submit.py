    ...
    try:
        ...
        if not all([briefing_id, feedback_type, rating]):
            return create_response(400, {
                'error': 'briefing_id, feedback_type, and rating are required'
            })
        
        # Validate feedback_type
        if feedback_type not in ['overall', 'article']:
            return create_response(400, {
                'error': 'feedback_type must be either "overall" or "article"'
            })
        ...
        if feedback_type == 'article' and article_position is None:
            return create_response(400, {
                'error': 'article_position is required for article feedback'
            })
        ...
        briefing_id_db, brew_id, execution_status, article_count, db_user_id, email = briefing_data
        ...
        if not user_id:
            ...
            cursor.close()
            conn.close()
            return create_response(403, {'error': 'User not authorized for this briefing'})
        ...
        if feedback_type == 'article':
            try:
                ...
                conn.close()
                return create_response(400, {
                    'error': 'article_position must be a valid integer'
                })
        ...
        """
        existing_params = [briefing_id, user_id, simple_feedback_type, article_position or 0]
        ...
        if existing_feedback:
            ...
            if feedback_type == 'article' and article_position is not None:
                ...
                if cache_result:
                    ...
                    if raw_articles and 0 < article_position <= len(raw_articles):
                        ...
                        article_title = article.get('headline', '')[:500]  # Truncate to fit column
                        article_source = article.get('source', '')[:200]   # Truncate to fit column
            ...
            cursor.execute("""
                ...
                RETURNING id
            """, (
                briefing_id, user_id, simple_feedback_type,
                article_position or 0, article_title, article_source,
                datetime.utcnow()
            ))
        ...
        return create_response(200, {
            ...
            'rating': rating,
            'article_position': article_position if feedback_type == 'article' else None,
            'action': action
        })
